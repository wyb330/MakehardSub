...
from subtitle.exceptions import MalformedCaptionError

TIMESTAMP_PATTERN = re.compile('(\d+)?:?(\d{2}):(\d{2})[.,](\d{1,3})')
...
class Caption(object):
    ...
    @staticmethod
    def to_timestamp(total_seconds):
    ...
    @property
    def start(self):
        return self.to_timestamp(self._start)
    ...
    @property
    def end(self):
        return self.to_timestamp(self._end)
